...
double ast_eval(ast* self) {
	...
	switch(self->tag) {
		...
		case ast_lshift: return (int)ast_eval(ast_first(self)) << (int)ast_eval(ast_second(self));
		case ast_rshift: return (int)ast_eval(ast_first(self)) >> (int)ast_eval(ast_second(self));
